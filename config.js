const CONFIG = {
    ...
    ORIENTATION_STORAGE_KEY: 'gymTracker_orientation',
    DATA_VERSION: 1,
    MAX_SERIES: 6,
    MAX_STARS: 5,
    LANGUAGE: 'auto',
...
function getCustomExercises(){var d=localStorage.getItem(CONFIG.CUSTOM_EXERCISES_KEY);return d?JSON.parse(d):{};}
function saveCustomExercises(data){localStorage.setItem(CONFIG.CUSTOM_EXERCISES_KEY,JSON.stringify(data));}
function addCustomExercise(categoryKey,exerciseName){var custom=getCustomExercises();if(!custom[categoryKey])custom[categoryKey]=[];if(custom[categoryKey].indexOf(exerciseName)===-1){custom[categoryKey].push(exerciseName);saveCustomExercises(custom);}}
function removeCustomExercise(categoryKey,exerciseName){var custom=getCustomExercises();if(!custom[categoryKey])return;custom[categoryKey]=custom[categoryKey].filter(function(n){return n!==exerciseName;});if(!custom[categoryKey].length)delete custom[categoryKey];saveCustomExercises(custom);}