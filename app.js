...
function addTraining(name,date,bodyWeight){var ts=getTrainings();var tr={id:generateId(),name:name,date:date,bodyWeight:bodyWeight||null,exercises:[]};ts.push(tr);saveTrainings(ts);return tr;}
...
function moveExercise(tid,from,to){var ts=getTrainings();var tr=ts.find(function(x){return x.id===tid;});if(!tr||from<0||to<0||from>=tr.exercises.length||to>=tr.exercises.length)return;var item=tr.exercises.splice(from,1)[0];tr.exercises.splice(to,0,item);saveTrainings(ts);}
...
function renderStarsReadonly(rating){var h='<span class="star-rating-readonly">';for(var i=1;i<=CONFIG.MAX_STARS;i++){if(rating>=i)h+='<span class="star-ro star-full">★</span>';else if(rating>=i-0.5)h+='<span class="star-ro star-half">★</span>';else h+='<span class="star-ro star-empty">★</span>';}return h+'</span>';}
...
function getRepsClass(total,type){
    if(total===0)return'';
    if(type){var cn=tCategory('cardio');if(type===cn)return'';}
...
function getOrientation(){return localStorage.getItem(CONFIG.ORIENTATION_STORAGE_KEY)||'auto';}
function applyOrientation(mode){
    localStorage.setItem(CONFIG.ORIENTATION_STORAGE_KEY,mode);
    if(!screen.orientation||!screen.orientation.lock)return;
    if(mode==='portrait'){screen.orientation.lock('portrait').catch(function(){});}
    else if(mode==='landscape'){screen.orientation.lock('landscape').catch(function(){});}
...
function initSettings(){
    ...
    document.getElementById('settingsOrientationLabel').textContent=t('settings_orientation');
    ...
    function renderExList(){var catKey=catSelect.value;exList.innerHTML='';if(!catKey)return;var custom=getCustomExercises();var customForCat=custom[catKey]||[];var builtinKeys=CONFIG.EXERCISE_DATA[catKey]||[];
        builtinKeys.forEach(function(exKey){var name=tExercise(exKey);var item=document.createElement('div');item.className='settings-ex-item';item.innerHTML='<span class="settings-ex-item-name">'+escapeHtml(name)+'</span><span class="settings-ex-item-badge">'+t('settings_builtin_label')+'</span>';exList.appendChild(item);});
        customForCat.forEach(function(name){var item=document.createElement('div');item.className='settings-ex-item';item.innerHTML='<span class="settings-ex-item-name">'+escapeHtml(name)+'</span><span class="settings-ex-item-badge custom">'+t('settings_custom_label')+'</span><button class="settings-ex-delete-btn" data-cat="'+catKey+'" data-name="'+escapeHtml(name)+'" title="'+t('btn_delete')+'">✕</button>';exList.appendChild(item);});
    ...
    function handleAddEx(){var catKey=catSelect.value;var name=nameInput.value.trim();if(!catKey){catSelect.focus();return;}if(!name){alert(t('settings_exercise_name_empty'));nameInput.focus();return;}
        var existingBuiltin=(CONFIG.EXERCISE_DATA[catKey]||[]).map(function(ek){return tExercise(ek).toLowerCase();});var existingCustom=(getCustomExercises()[catKey]||[]).map(function(n){return n.toLowerCase();});
        if(existingBuiltin.indexOf(name.toLowerCase())!==-1||existingCustom.indexOf(name.toLowerCase())!==-1){alert(t('settings_exercise_exists'));nameInput.focus();return;}
    ...
    exList.addEventListener('click',function(e){var db=e.target.closest('.settings-ex-delete-btn');if(!db)return;var catKey=db.dataset.cat;var name=db.dataset.name;if(confirm(t('confirm_delete_custom_exercise',{name:name}))){removeCustomExercise(catKey,name);renderExList();}});
}
...
function initIndexPage(){
    var body=document.getElementById('trainingsBody');var modal=document.getElementById('trainingModal');var form=document.getElementById('trainingForm');
    var mTitle=document.getElementById('trainingModalTitle');var nIn=document.getElementById('trainingName');var dIn=document.getElementById('trainingDate');var wIn=document.getElementById('trainingWeight');
    var empty=document.getElementById('emptyMessage');var fN=document.getElementById('filterName');var fDF=document.getElementById('filterDateFrom');var fDT=document.getElementById('filterDateTo');
    var fBar=document.getElementById('filtersBar');var tFBtn=document.getElementById('toggleFiltersBtn');var selAll=document.getElementById('selectAllCheckbox');
    var selBar=document.getElementById('selectionToolbar');var selCnt=document.getElementById('selectionCount');
    var pdfBtn=document.getElementById('downloadPdfBtn');var editBtn=document.getElementById('editSelectedBtn');var delBtn=document.getElementById('deleteSelectedBtn');var clrBtn=document.getElementById('clearSelectionBtn');
    ...
    document.getElementById('exportJsonBtn').textContent=t('btn_export_json');document.getElementById('importJsonBtn').textContent=t('btn_import_json');
    ...
    document.getElementById('clearFiltersBtn').textContent=t('btn_clear_filters');clrBtn.textContent=t('btn_clear_selection');selAll.title=t('select_all_title');
    ...
    document.querySelector('#importModal h2').textContent=t('import_title');document.querySelector('.import-warning').textContent=t('import_warning');
    document.getElementById('importCancelBtn').textContent=t('btn_cancel');document.getElementById('importMergeBtn').textContent=t('btn_merge');document.getElementById('importReplaceBtn').textContent=t('btn_replace');
    ...
    function filtered(){
        ...
        ts.sort(function(a,b){var va,vb;
    ...
    function updSort(){['sortIconName','sortIconDate','sortIconWeight','sortIconExercises','sortIconRating'].forEach(function(id){document.getElementById(id).textContent='';});var map={name:'sortIconName',date:'sortIconDate',weight:'sortIconWeight',exercises:'sortIconExercises',rating:'sortIconRating'};if(map[sortCol])document.getElementById(map[sortCol]).textContent=sortDir==='asc'?'▲':'▼';}
    ...
    function render(){
        ...
        ts.forEach(function(tr){var row=document.createElement('tr');row.dataset.id=tr.id;var wt=tr.bodyWeight?tr.bodyWeight+' '+t('weight_unit'):'-';
    ...
    function exportPDF(ids){
        ...
        trainings.forEach(function(tr,ti){
            ...
            doc.setTextColor(60);doc.setFontSize(11);doc.text(t('pdf_total_exercises')+': '+tr.exercises.length,14,y);
            var gt=0;tr.exercises.forEach(function(e){gt+=getTotalReps(e);});doc.text(t('pdf_total_reps')+': '+gt,100,y);
            if(tr.bodyWeight)doc.text(t('pdf_body_weight')+': '+tr.bodyWeight+' '+t('weight_unit'),190,y);y+=8;
            if(!tr.exercises.length){doc.setFontSize(12);doc.setTextColor(150);doc.text(t('pdf_no_exercises'),pw/2,y+10,{align:'center'});}
            else{var grp={};tr.exercises.forEach(function(e){var tp=e.type||t('pdf_uncategorized');if(!grp[tp])grp[tp]=[];grp[tp].push(e);});
                Object.keys(grp).forEach(function(tp){doc.setFontSize(13);doc.setFont('helvetica','bold');doc.setTextColor(40);doc.text(tp,14,y);y+=2;
        ...
        var tp2=doc.internal.getNumberOfPages();for(var p=1;p<=tp2;p++){doc.setPage(p);doc.setFontSize(8);doc.setTextColor(180);doc.text(t('pdf_footer',{date:new Date().toLocaleDateString(dateLocale()),page:p,total:tp2}),pw/2,doc.internal.pageSize.getHeight()-10,{align:'center'});}
    ...
    form.addEventListener('submit',function(e){e.preventDefault();var name=nIn.value.trim(),date=dIn.value,weight=wIn.value?parseFloat(wIn.value):null;if(!name||!date)return;if(editingId){updateTraining(editingId,{name:name,date:date,bodyWeight:weight});closeM();render();}else{var tr=addTraining(name,date,weight);closeM();window.location.href='training.html?id='+tr.id;}});
...
function initTrainingPage(){
    var EXDATA=getLocalizedExerciseData();var params=new URLSearchParams(window.location.search);var trainingId=params.get('id');
    if(!trainingId){window.location.href='index.html';return;}
    var exBody=document.getElementById('exercisesBody');var empty=document.getElementById('emptyMessage');
    ...
    function getFilteredEx(tr){
        ...
        var q=fEx.value.trim().toLowerCase();if(q)exs=exs.filter(function(e){return(e.name||'').toLowerCase().includes(q);});
        if(exSortCol){exs.sort(function(a,b){var va,vb;
            ...
            return va<vb?(exSortDir==='asc'?-1:1):va>vb?(exSortDir==='asc'?1:-1):0;});}return exs;
    }
    ...
    document.getElementById('addExerciseBtn').addEventListener('click',function(){
        ...
        addExercise(trainingId,{type:'',name:'',load:'',series:emptySeries,rating:0});renderEx();
        var rows=exBody.querySelectorAll('tr');if(rows.length){var sel2=rows[rows.length-1].querySelector('.inline-type');if(sel2)sel2.focus();}
